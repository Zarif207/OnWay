...
export default function RiderInboxPage() {
    ...
    const fetchChats = useCallback(async () => {
        ...
        try {
            ...
            // Auto-select first chat if nothing selected
            if (!selected && rideChats.length > 0) {
                if (window.innerWidth >= 768) {
                    setSelected(rideChats[0]);
                }
            }
        } catch (err) {
            console.error(err);
    ...
    const filteredChats = useMemo(() => {
        ...
        let filtered = chats.filter(
            (c) => c.senderName?.toLowerCase().includes(query)
        );
        if (supportChat && (supportChat.senderName.toLowerCase().includes(query) || !query)) {
    ...
    return (
        <div className="flex h-screen text-gray-700 font-sans overflow-hidden p-4">
            <div className="max-w-[1700px] w-full mx-auto bg-white md:rounded-[2.5rem] flex overflow-hidden border border-gray-100 relative h-full">
                ...
                {/* SIDEBAR */}
                <aside
                    className={`${showMobileList ? "flex" : "hidden md:flex"} w-full md:w-[200px] lg:w-[350px] flex-col border-r border-gray-50 shrink-0 h-full`}
                >
                            ...
                            filteredChats.map((c) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full p-5 mb-2 flex items-center gap-4 rounded-[2.2rem] transition-all relative ${selected?.roomId === c.roomId
                                            ? "bg-white shadow-lg border-emerald-50"
                                            : "hover:bg-emerald-50/50"
                                            }`}
                                    >
                                        <div className="relative shrink-0">
                                            <div
                                                className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black uppercase ${isSupport ? "bg-purple-50 text-purple-600" : "bg-emerald-50 text-emerald-600"
                                                    }`}
                                            >
                                        ...
                                        <div className="flex-1 text-left min-w-0">
                                            ...
                                            <p className={`text-xs truncate font-bold ${c.unreadCount > 0 ? "text-emerald-600" : "text-gray-400"}`}>
                        ...
                        <>
                            <header className="h-24 px-8 border-b border-gray-50 flex items-center justify-between shrink-0">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center font-black text-emerald-600 border border-emerald-100 uppercase">
                                    ...
                                    <div>
                                        ...
                                        {selected.type !== "support" && (
                                            <span className="text-[9px] font-black tracking-widest text-emerald-500 uppercase">
                                                {onlineStatus[selected.passengerId] === "online" ? "Online" : "Offline"}
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-8 space-y-6 bg-[#FAFAFF]/40">
                                {messagesLoading && messages.length === 0 ? (
                                    ...
                                    messages.map((m, i) => (
                                        <div key={m._id || i} className={`flex ${m.senderId === user._id ? "justify-end" : "justify-start"}`}>
                                            <div className={`max-w-[80%] p-5 rounded-[2rem] shadow-sm ${m.senderId === user._id
                                                ? "bg-emerald-600 text-white rounded-tr-none"
                                                : "bg-white border border-gray-100 text-gray-700 rounded-tl-none"
                                                }`}>
                                                {m.messageType === "image" ? (
                                                    <img src={m.fileUrl} className="rounded-xl max-h-60 object-cover" />
                                                ) : (
                                                    <p className="font-bold text-sm">{m.message}</p>
                                                )}
                                ...
                                {typingUser && (
                                    <div className="text-[10px] font-black text-emerald-600 animate-pulse uppercase tracking-widest">
                            ...
                            <footer className="p-6 border-t border-gray-50">
                                {sendError && (
                                    <div className="mb-3 p-3 bg-red-50 text-red-600 rounded-2xl border border-red-100 flex items-center justify-between text-xs font-bold">
                                        ...
                                        <button onClick={clearSendError} className="px-3 py-1 bg-red-100 rounded-xl font-black text-[10px]">
                                            Dismiss
                                        </button>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center gap-3 bg-gray-50 p-2 rounded-[2.5rem] border border-gray-100">
                                    ...
                                    />
                                    <button onClick={() => handleSend()} disabled={!input.trim() && !isUploading} className="bg-emerald-600 text-white p-4 rounded-full disabled:bg-gray-200 shadow-lg">
                                        <Send size={20} />
                                    </button>
                                </div>
                            </footer>
