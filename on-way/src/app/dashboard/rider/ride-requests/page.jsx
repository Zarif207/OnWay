...
import {
    MapPin, Navigation, User, Search, Loader2,
...
const RideRequestMap = dynamic(() => import("./_components/RideRequestMap"), {
    ssr: false,
    loading: () => (
        <div className="h-full w-full bg-gray-100 animate-pulse flex items-center justify-center rounded-[2rem]">
...
export default function RideRequestsPage() {
    ...
    // Fetch existing searching rides on mount
    useEffect(() => {
            ...
            try {
                ...
                if (res.data.success) {
                    const pendingRides = res.data.data.map((ride) => ({
                        ...ride,
                        bookingId: ride._id,
                        pickupLocation: ride.pickupLocation?.address || ride.pickupLocation,
                        dropLocation: ride.dropoffLocation?.address || ride.dropoffLocation,
                        pickupCoords: [ride.pickupLocation?.lat || 23.8103, ride.pickupLocation?.lng || 90.4125],
                        dropCoords: [ride.dropoffLocation?.lat || 23.8213, ride.dropoffLocation?.lng || 90.4195],
                        fare: ride.price,
                        passengerName: ride.passengerName || "Passenger",
                        expiresAt: new Date(ride.createdAt).getTime() + 60000,
                    }));
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setRequests((prev) => prev.filter((r) => r.expiresAt > Date.now()));
        }, 1000);
    ...
    // Open price offer modal
    const handleOpenOffer = (ride) => {
        setSelectedRide(ride);
        setOfferedPrice(String(ride.fare || ""));
        setShowPriceModal(true);
    };

    // Send price offer to passenger via socket
    const handleSendOffer = () => {
        if (!selectedRide || !offeredPrice) return;
        const socket = getRiderSocket(riderId);
        ...
        toast.success("Price offer sent to passenger!", {
            icon: "💸",
            style: { borderRadius: "15px", background: "#011421", color: "#fff" },
        });
    };

    // Direct accept without price negotiation
    const handleAccept = (bookingId) => {
        if (!riderId) return;
        const socket = getRiderSocket(riderId);

        socket.emit("ride:accept", { bookingId, riderId });

        toast.success("Ride Accepted! Connecting...", {
            icon: "🚀",
            style: { borderRadius: "15px", background: "#011421", color: "#fff" },
        });

        setTimeout(() => {
            router.push(`/dashboard/rider/ongoing-ride?bookingId=${bookingId}`);
        }, 1200);
    };

    const handleReject = (rideId) => {
        setRequests((prev) => prev.filter((r) => (r.bookingId || r._id) !== rideId));
        ...
        <div className="space-y-10 pb-20">
                ...
                <div className="space-y-2 relative z-10">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                            {requests.length} active request{requests.length !== 1 ? "s" : ""} nearby
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <AnimatePresence mode="popLayout">
                    {!isLoadingRides && requests.length > 0 ? (
                        requests.map((ride) => (
                            <RideRequestCard
                                key={ride.bookingId || ride._id}
                                ride={ride}
                                onAccept={() => handleAccept(ride.bookingId || ride._id)}
                                onOffer={() => handleOpenOffer(ride)}
                        ...
                        <motion.div
                            ...
                            className="lg:col-span-2 h-[500px] flex flex-col items-center justify-center space-y-8 bg-white rounded-[3rem] border border-gray-100 shadow-sm border-dashed"
                        >
                            ...
                            <div className="text-center space-y-3">
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest max-w-[250px] mx-auto leading-relaxed">
                                    Stay online to receive instant notifications for new ride requests.
                                </p>
                            </div>
                        </motion.div>
                    ) : null}
                    ...
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                        ...
                        >
                            <div className="space-y-1">
                                <p className="text-[10px] font-black text-primary uppercase tracking-widest">Price Negotiation</p>
                                <h2 className="text-2xl font-black text-[#011421] tracking-tight">Make an Offer</h2>
                                <p className="text-xs text-gray-500 font-bold">
                                    Passenger requested fare: <span className="text-[#011421]">৳{selectedRide.fare}</span>
                                </p>
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Your Offered Price</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-16 pl-12 pr-5 bg-gray-50 border border-gray-200 rounded-2xl text-xl font-black text-[#011421] focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all"
                            ...
                            <div className="flex gap-3">
                                {[
                                    Math.round(selectedRide.fare * 0.9),
                                    selectedRide.fare,
                                    Math.round(selectedRide.fare * 1.1),
                                ].map((price) => (
                                    <button
                                        key={price}
                                        onClick={() => setOfferedPrice(String(price))}
                                        className={`flex-1 py-3 rounded-xl text-xs font-black border transition-all ${Number(offeredPrice) === price
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-2">
                                <button
                                    onClick={() => { setShowPriceModal(false); setSelectedRide(null); }}
                                    className="h-14 rounded-2xl border border-gray-200 text-gray-500 font-black uppercase tracking-widest text-xs hover:bg-gray-50 transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={handleSendOffer}
                                    disabled={!offeredPrice || Number(offeredPrice) <= 0}
                                    className="h-14 bg-primary text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                    <DollarSign size={16} />
                                    Send Offer
                                </button>
...
function RideRequestCard({ ride, onAccept, onOffer, onReject }) {
    const [timeLeft, setTimeLeft] = useState(() =>
        Math.max(0, Math.ceil((ride.expiresAt - Date.now()) / 1000))
    );

    useEffect(() => {
        const interval = setInterval(() => {
            const rem = Math.max(0, Math.ceil((ride.expiresAt - Date.now()) / 1000));
            setTimeLeft(rem);
    ...
    return (
        ...
        >
            ...
            <div className="h-56 relative rounded-[2rem] overflow-hidden m-3 border border-gray-50 shadow-inner group-hover:border-primary/20 transition-all">
                ...
                {/* Countdown */}
                <div className={`absolute bottom-3 left-3 z-[400] flex items-center gap-2.5 backdrop-blur-md px-4 py-2 rounded-xl border shadow-xl transition-colors ${isExpiring ? "bg-red-900/95 border-red-500/20" : "bg-[#011421]/95 border-white/10"
                    }`}>
                    <div className="relative h-7 w-7">
                        <svg className="h-full w-full" viewBox="0 0 36 36">
                            <circle cx="18" cy="18" r="16" fill="none" className="stroke-white/10" strokeWidth="3" />
                            <path
                                className={isExpiring ? "stroke-red-400" : "stroke-primary"}
                                strokeWidth="3"
                    ...
                    <span className="text-[8px] font-bold text-white uppercase tracking-widest">
                        {isExpiring ? "Expiring!" : "Expires In"}
                    </span>
                </div>
            </div>
            ...
            <div className="p-7 pt-4 space-y-6">
                ...
                <div className="space-y-5">
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Pickup</p>
                ...
                <div className="grid grid-cols-2 gap-4 bg-[#f5f7fa] rounded-2xl p-4 border border-gray-50">
                    <div className="space-y-0.5">
                        ...
                        <p className="text-base font-black text-[#011421] tracking-tighter">{ride.distance || "—"} km</p>
                    </div>
                    <div className="space-y-0.5 text-right">
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Est. Time</p>
                        <p className="text-base font-black text-[#011421] tracking-tighter">{ride.duration || "—"} min</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-2">
                    <button
                        onClick={onReject}
                        className="h-14 rounded-2xl border-2 border-gray-100 text-gray-400 font-black uppercase tracking-widest text-[9px] hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all flex items-center justify-center gap-1.5"
                    >
                        <XCircle size={13} />
                        Decline
                    </button>
                    <button
                        onClick={onOffer}
                        className="h-14 bg-amber-500 text-white rounded-2xl font-black uppercase tracking-widest text-[9px] shadow-lg shadow-amber-500/20 hover:bg-amber-600 transition-all flex items-center justify-center gap-1.5"
                    >
                        <DollarSign size={13} />
                        Offer Price
                    </button>
                    <button
                        onClick={onAccept}
                        className="h-14 bg-[#011421] text-white rounded-2xl font-black uppercase tracking-widest text-[9px] shadow-xl shadow-[#011421]/20 hover:bg-primary transition-all flex items-center justify-center gap-1.5"
                    >
