...
const Login = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const allUsers = JSON.parse(localStorage.getItem("allUsers") || "[]");
            const userFound = allUsers.find(u => u.email === data.email && u.password === data.password);

            if (userFound) {
                const result = await signIn("credentials", { email: data.email, password: data.password, redirect: false });
                if (result?.error) toast.error("Login failed!", { id: toastId });
                else {
            ...
            >
                <div className="w-full max-w-md">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                ...
                                <input {...register("email", { required: true })} type="email" placeholder="name@onway.com" className="input input-bordered w-full h-14 pl-12 rounded-2xl focus:border-secondary focus:ring-0 transition-all bg-gray-50/50" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <Link href="/forgot-password" size={11} className="text-[11px] font-bold text-secondary hover:underline">Forgot?</Link>
                            </div>
                            <div className="relative">
                                <FaLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" />
                                <input {...register("password", { required: true })} type={showPassword ? "text" : "password"} placeholder="••••••••" className="input input-bordered w-full h-14 pl-12 rounded-2xl focus:border-secondary focus:ring-0 transition-all bg-gray-50/50" />
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            disabled={loading}
                            className="btn btn-secondary w-full text-white rounded-2xl h-12 uppercase tracking-[0.2em] font-black shadow-2xl shadow-secondary/30 mt-4 border-none transition-all"
                        >
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <motion.button whileHover={{ y: -2 }} onClick={() => signIn("google")} className="btn flex-1 bg-white border-gray-100 rounded-2xl py-2 hover:bg-gray-50 h-12 shadow-sm uppercase text-[11px] font-bold transition-all">Google</motion.button>
                        <motion.button whileHover={{ y: -2 }} onClick={() => signIn("github")} className="btn flex-1 bg-neutral text-white border-none rounded-2xl py-2 hover:bg-black h-12 shadow-sm uppercase text-[11px] font-bold transition-all"><FaGithub className="mr-2" /> GitHub</motion.button>
                    </div>
