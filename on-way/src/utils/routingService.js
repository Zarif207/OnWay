...
export const getDrivingRoute = async (pickup, dropoff) => {
  ...
  if (
    ...
    Math.abs(pickup.lon) > 180 ||
    Math.abs(dropoff.lat) > 90 ||
    Math.abs(dropoff.lon) > 180
  ) {
  ...
  if (!hasNetwork) {
    console.warn(
      "⚠️ Network connectivity issues detected, using fallback route",
    );
  ...
  try {
    ...
    for (let i = 0; i < OSRM_ENDPOINTS.length; i++) {
      ...
      try {
        console.log(
          `🔄 Trying OSRM endpoint ${i + 1}/${OSRM_ENDPOINTS.length}: ${baseUrl}`,
        );
        ...
        console.log(
          `✅ Route calculated successfully using endpoint: ${baseUrl}`,
        );
